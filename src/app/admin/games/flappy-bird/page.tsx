...
import FlappyBird from "@/src/components/games/flappy-bird/FlappyBird";
...
const AdminFlappyBirdContent = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const playerData = new Map<
        string,
        { scores: number[]; name: string; lastPlayed?: Date }
      >();
  ...
  const resetAllScores = async () => {
    if (
      !confirm(
        "Are you sure you want to delete ALL Flappy Bird scores? This action cannot be undone!",
      )
    ) {
      return;
    }
                ...
                <CardContent className="space-y-4">
                  <div className="p-3 sm:p-4 border border-red-200 bg-red-50 dark:bg-red-950 dark:border-red-800 rounded-lg">
                    ...
                    <p className="text-xs sm:text-sm text-red-600 dark:text-red-300 mb-3">
                      This will permanently delete all player scores and reset
                      the leaderboard. Current data:{" "}
                      {stats?.totalGamesPlayed || 0} games from{" "}
                ...
                <CardContent className="space-y-4">
                  <div className="p-3 sm:p-4 border border-blue-200 bg-blue-50 dark:bg-blue-950 dark:border-blue-800 rounded-lg">
                    ...
                    <p className="text-xs sm:text-sm text-blue-600 dark:text-blue-300 mb-3">
                      Reset the lives system to 5/5 for testing game mechanics.
                      This only affects your current browser session.
                    </p>
                    <Button
                      onClick={resetLives}
                      variant="outline"
