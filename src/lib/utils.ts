...
export function calculatePlayerKDs(
  playerKills: number[],
...
export function calculateOverallPlayerKD(
  ...
  Object.values(matchScores).forEach((score: any) => {
    ...
    const participated = score.playerParticipation
      ? score.playerParticipation[playerIndex]
      : true;
  ...
  return {
    ...
    avgKillsPerMatch:
      matchesParticipated > 0
        ? parseFloat((totalKills / matchesParticipated).toFixed(1))
        : 0,
  };
}
...
export function calculateTiebreakerData(team: any): TeamTiebreakerData {
  ...
  const matchNumbers = Object.keys(matchScores)
    .map(Number)
    .sort((a, b) => a - b);
...
export async function getTournamentsWithTeams(
  ...
  for (const tournamentId of tournamentIds) {
    try {
      ...
      console.error(
        `Error checking teams for tournament ${tournamentId}:`,
        error
      );
    }
  }
...
export async function getBestTournamentForAutoSelect(
