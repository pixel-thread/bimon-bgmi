...
export default function FlappyBird() {
  ...
  useEffect(() => {
    ...
    if (storedLives !== null) {
      ...
      const parsedLastLifeLoss = storedLastLifeLoss
        ? parseInt(storedLastLifeLoss)
        : null;
      if (parsedLives < MAX_LIVES && parsedLastLifeLoss) {
        const timePassed = Date.now() - parsedLastLifeLoss;
        const livesToRegenerate = Math.floor(
          timePassed / LIFE_REGENERATION_TIME
        );
        const newLives = Math.min(MAX_LIVES, parsedLives + livesToRegenerate);
        setLives(newLives);
        localStorage.setItem(LIVES_STORAGE_KEY, newLives.toString());
        if (newLives < MAX_LIVES) {
          const adjustedLastLifeLoss =
            parsedLastLifeLoss + livesToRegenerate * LIFE_REGENERATION_TIME;
          setLastLifeLoss(adjustedLastLifeLoss);
          localStorage.setItem(
            LAST_LIFE_LOSS_KEY,
            adjustedLastLifeLoss.toString()
          );
        } else {
  ...
  useEffect(() => {
    ...
    const initialTimeUntilNext =
      LIFE_REGENERATION_TIME - (timeSinceLastLoss % LIFE_REGENERATION_TIME);
    ...
    const interval = setInterval(
      () => {
        ...
        const timeUntilNext =
          ...
          (currentTimeSinceLastLoss % LIFE_REGENERATION_TIME);
      ...
      while (accumulator >= FIXED_FRAME_TIME) {
        ...
        if (isDying) {
          ...
          setPipes((prevPipes) => {
            ...
            return updatedPipes.length > MAX_PIPES
              ? updatedPipes.slice(-MAX_PIPES)
              : updatedPipes;
          });
          ...
          if (canSpawn && currentTime - lastSpawnTime >= spawnInterval) {
            ...
            const dynamicMaxGap = Math.min(
              GAME_HEIGHT - pipeGap - 80,
              maxGap - scoreBias
            );
            const gapTop =
              Math.random() * (dynamicMaxGap - dynamicMinGap) + dynamicMinGap;
    ...
    <div className="flex flex-col items-center justify-start min-h-screen bg-gray-900 pt-0">
      ...
      >
        ...
        {lives === 0 && !gameOver && (
          ...
          >
            <p className="text-2xl font-pixel text-white mb-4 retro-shadow">
              OUT OF LIVES
            </p>
        ...
        {!gameStarted && !gameOver && lives > 0 && (
          ...
          >
            ...
            <p className="text-xl font-pixel text-white retro-shadow animate-pulse">
              TAP TO START
            </p>
          </motion.div>
        )}
        ...
        {gameOver && (
          ...
          >
            <div className="p-6">
              ...
              {lives === 0 ? (
                <>
                  <p className="text-sm font-pixel text-white retro-shadow mt-2">
                    OUT OF LIVES
                  </p>
        ...
        {pipes.map((pipe) => (
          <motion.div
            ...
            style={{ x: pipe.x }}
            animate={pipeControls}
          >
            {/* Top Pipe */}
            <div
              className="absolute"
              style={{ width: PIPE_WIDTH, height: pipe.gapTop, top: 0 }}
            >
              <Image
            ...
            <div
              ...
              style={{
                width: PIPE_WIDTH,
                height:
                  GAME_HEIGHT - (pipe.gapTop + getDynamicValues(score).pipeGap),
        ...
        {!gameOver && (
          <div className="absolute top-4 left-1/2 transform -translate-x-1/2 text-center z-10">
            <div className="text-4xl font-pixel text-white retro-shadow">
              {score}
            </div>
            {user?.uid && bestScore > 0 && (
              <div className="text-xs font-pixel text-yellow-300 retro-shadow mt-1">
                BEST: {bestScore}
              </div>
            )}
          </div>
      ...
      <div className="mt-8 max-w-md mx-auto text-center px-4 mb-8">
        <h2 className="text-2xl font-pixel text-white mb-4 retro-shadow">
          FLAPPY BIRD CHALLENGE
        </h2>
        <p className="text-gray-300 mb-4 font-pixel text-xs leading-relaxed">
          NAVIGATE THE BIRD THROUGH PIPES BY TIMING YOUR JUMPS PERFECTLY. CLICK
          OR PRESS SPACE TO FLAP UPWARD. AVOID HITTING PIPES OR GROUND!
        </p>
