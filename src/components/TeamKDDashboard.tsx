...
import { getPlayerStats, calculateOverallPlayerKD } from "@/src/lib/utils";
...
export default function TeamKDDashboard({
  teams,
  onUpdatePlayerStats,
}: TeamKDDashboardProps) {
  ...
  const getTeamOverallStats = (team: any) => {
    ...
    Object.values(matchScores).forEach((score: any) => {
      if (score.playerKills && score.playerDeaths) {
        totalKills += score.playerKills.reduce(
          (sum: number, kills: number) => sum + kills,
          0
        );
        totalDeaths += score.playerDeaths.reduce(
          (sum: number, deaths: number) => sum + deaths,
          0
        );
        matchesPlayed++;
      }
    ...
    return {
      ...
      avgKillsPerMatch:
        matchesPlayed > 0
          ? parseFloat((totalKills / matchesPlayed).toFixed(1))
          : 0,
    };
  };
  ...
  const getTopPerformers = () => {
    ...
    return allPlayerStats
      .sort((a, b) => b.overallKD - a.overallKD)
      .slice(0, 10);
  };
    ...
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="p-4">
              ...
              <p className="text-2xl font-bold text-green-600">
                {teams.reduce(
                  (sum, team) => sum + getTeamOverallStats(team).totalKills,
                  0
                )}
              </p>
            </Card>
            ...
            <Card className="p-4">
              ...
              <p className="text-2xl font-bold text-red-600">
                {teams.reduce(
                  (sum, team) => sum + getTeamOverallStats(team).totalDeaths,
                  0
                )}
              </p>
            </Card>
              ...
              <p className="text-2xl font-bold text-blue-600">
                {teams.length > 0
                  ? (
                      teams.reduce(
                        (sum, team) =>
                          sum + getTeamOverallStats(team).overallKD,
                        0
                      ) / teams.length
                    ).toFixed(2)
                  : "0.00"}
            ...
            return (
              <Card key={team.id} className="p-4">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Badge
                    variant={
                      teamStats.overallKD >= 2
                        ? "default"
                        : teamStats.overallKD >= 1
                        ? "secondary"
                        : "outline"
                    }
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-xs">
                              K:{" "}
                              {score.playerKills?.reduce(
                                (sum: number, k: number) => sum + k,
                                0
                              ) || 0}{" "}
                              | D:{" "}
                              {score.playerDeaths?.reduce(
                            ...
                            <Button
                              ...
                              variant="ghost"
                              onClick={() =>
                                handleEditPlayerStats(team, matchNum)
                              }
      ...
      {selectedTeam && (
        <PlayerKDEditModal
          ...
          initialPlayerKills={
            selectedTeam.matchScores?.[selectedMatch]?.playerKills?.map(
              (k: number) => k.toString()
            ) || []
          }
          initialPlayerDeaths={
            selectedTeam.matchScores?.[selectedMatch]?.playerDeaths?.map(
              (d: number) => d.toString()
            ) || []
          }
