...
export class SocialAuthService {
  ...
  static async linkSocialAccount(
    ...
    try {
      ...
      if (userRole && ['admin', 'super_admin'].includes(userRole) && linkedUser.email) {
        await this.updateAdminAuthorization(linkedUser.email, userRole, provider.id);
      }
      ...
      if (error.code === 'auth/provider-already-linked') {
        ...
        return {
          success: false,
          message: 'An account with this email already exists. Please sign in with that account first.',
        };
   ...
   */
  static async getLinkedAccounts(user: User): Promise<Record<string, LinkedAccount>> {
    try {
