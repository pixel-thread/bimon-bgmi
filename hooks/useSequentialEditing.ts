...
import { calculatePlacementPoints } from "@/lib/utils";

export function useSequentialEditing(teams: CombinedTeamData[], initialMatch: string) {
  ...
  const matchOptions = Array.from(
    {
      length: teams.length
        ? Math.max(
            ...teams.flatMap((team) => Object.keys(team.matchScores || {}).map(Number)),
            1
          )
        : 1,
    },
  ...
  useEffect(() => {
    const initialEdits: { [teamId: string]: { placement?: string; kills?: string; teamName?: string; playerKills?: string[]; playerParticipation?: boolean[] } } = {};
    teams.forEach((team) => {
      const matchScore = team.matchScores?.[sequentialMatch] || { kills: 0, placementPoints: 0, playerKills: [], playerParticipation: [] };
      const position = matchScore.placementPoints > 0 ? getPositionFromPoints(matchScore.placementPoints) : "";
      initialEdits[team.id] = {
        placement: position ? String(position) : "",
        kills: matchScore.kills > 0 ? String(matchScore.kills) : "",
        teamName: team.teamName,
        playerKills: matchScore.playerKills?.map(k => k > 0 ? String(k) : "") || [],
        // Smart defaults: Default to participated if no data exists
        playerParticipation: matchScore.playerParticipation || team.players.map(() => true),
      };
    });
  ...
  const setTempEdit = (teamId: string, field: "placement" | "kills" | "teamName" | "playerKills" | "playerParticipation", value: string | string[] | boolean[] | undefined) => {
    setTempEdits((prev) => ({
      ...prev,
      [teamId]: {
        ...prev[teamId],
  ...
  const saveEdits = useCallback(async () => {
    for (const team of teams) {
      ...
      if (!updatedScores[sequentialMatch]) {
        updatedScores[sequentialMatch] = { kills: 0, placementPoints: 0, playerKills: [], playerKD: [], playerParticipation: [] } as any;
      }

      // Apply explicit edits
      if (teamEdits.placement !== undefined) {
        const position = parseInt(teamEdits.placement) || 0;
        updatedScores[sequentialMatch].placementPoints = calculatePlacementPoints(position);
      }
      ...
      if (teamEdits.playerKills !== undefined) {
        updatedScores[sequentialMatch].playerKills = teamEdits.playerKills.map(k => parseInt(k) || 0);
      }

      if (teamEdits.playerParticipation !== undefined) {
        updatedScores[sequentialMatch].playerParticipation = teamEdits.playerParticipation;
      }
      ...
      const participation = Array.isArray(ms.playerParticipation) && ms.playerParticipation.length > 0
        ? ms.playerParticipation
        : team.players.map(() => true);
      const normalizedParticipation = participation.slice(0, targetLen).concat(Array(Math.max(0, targetLen - participation.length)).fill(true));
      ms.playerParticipation = normalizedParticipation;

      const pk = Array.isArray(ms.playerKills) ? ms.playerKills : [];
      const normalizedPK = pk.slice(0, targetLen).concat(Array(Math.max(0, targetLen - pk.length)).fill(0));
      ms.playerKills = normalizedPK;

      // Recalculate per-player KD using participation
      const playerKDs = ms.playerKills.map((kills: number, idx: number) => (ms.playerParticipation[idx] ? parseFloat((kills / 1).toFixed(1)) : 0));
  ...
  const hasUnsavedChanges = useCallback(() => {
    for (const team of teams) {
      ...
      const matchScore = team.matchScores?.[sequentialMatch] || { kills: 0, placementPoints: 0, playerKills: [], playerParticipation: [] };
      const savedPlacement = matchScore.placementPoints > 0 ? String(getPositionFromPoints(matchScore.placementPoints)) : "";
      const savedKills = matchScore.kills > 0 ? String(matchScore.kills) : "";
      const savedPlayerKills = matchScore.playerKills?.map(k => k > 0 ? String(k) : "") || [];
      
      // More robust comparison for player participation - handle undefined/null values properly
      const savedPlayerParticipation = matchScore.playerParticipation && matchScore.playerParticipation.length > 0 
      ...
      const playerKillsChanged = teamEdits.playerKills && 
        JSON.stringify(normalizePlayerKills(teamEdits.playerKills)) !== JSON.stringify(normalizePlayerKills(savedPlayerKills));
      
      // Better participation comparison - ensure same length and values
      const participationChanged = teamEdits.playerParticipation && 
        (teamEdits.playerParticipation.length !== savedPlayerParticipation.length ||
         teamEdits.playerParticipation.some((val, idx) => val !== savedPlayerParticipation[idx]));

      if (placementChanged || killsChanged || teamNameChanged || playerKillsChanged || participationChanged) {
        return true;
      }
    }
    return false;
