...
export default function MobileTable({ teams, getScore }: MobileTableProps) {
  ...
  const maxHeight = teams.length > 30 ? 'max-h-[550px]' : teams.length > 20 ? 'max-h-[500px]' : 'max-h-[450px]';
  
  return (
    <div className="flex justify-center w-full px-4">
      <div className={`mobile-table ${maxHeight} relative border border-gray-600 rounded-lg w-full max-w-sm mx-auto bg-gray-900/80`}>
          ...
          <Table className="table-fixed w-full">
            ...
            <TableBody className="text-white">
              {teams.map((team, index) => {
                ...
                const teamDisplayName = `${team.teamName}${score.chickens > 0 ? ` 🍗 ${score.chickens}` : ''}`;
        ...
        <style jsx>{`
          ...
          .animate-marquee {
            animation: smooth-marquee 5s cubic-bezier(0.25, 0.46, 0.45, 0.94) infinite;
