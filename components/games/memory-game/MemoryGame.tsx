...
export default function MemoryGame() {
  ...
  const [shuffledPositions, setShuffledPositions] = useState<{x:number;y:number}[]>([]);
  ...
  const initializeGameAfterAd = () => {
    ...
    setTimeout(() => {
      ...
      CARD_SYMBOLS.forEach((symbol) => {
        gameCards.push({ id: id++, value: symbol, isFlipped: false, isMatched: false });
        gameCards.push({ id: id++, value: symbol, isFlipped: false, isMatched: false });
      });
  ...
  const startTimer = () => {
    ...
    timerRef.current = setInterval(() => {
      setGameTime(prev => (prev >= 999 ? (clearInterval(timerRef.current!), 999) : prev + 1));
    }, 1000);
  };
  ...
  const handleCardClick = (cardId: number) => {
    ...
    if (newFlipped.length === 2) {
      ...
      if (firstCard?.value === secondCard?.value) {
        setTimeout(() => {
          setCards(prev =>
            prev.map(c => (newFlipped.includes(c.id) ? { ...c, isMatched: true } : c))
          );
  ...
  return (
    <div className="flex flex-col items-center justify-start min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 dark:from-gray-900 dark:to-gray-800 p-4 pt-0">
      <div className="w-full max-w-4xl">
        <p className="text-gray-600 dark:text-gray-300 text-center mb-4">Match all the pairs to win!</p>
        {showTopBanner && (
      ...
      <div className="flex gap-4 sm:gap-6 mb-4 text-center">
        <div
          className={`
            ...
            ${isNewHighScore ? "animate-pulse ring-4 ring-yellow-400 dark:ring-yellow-300" : ""}
          `}
        >
          <div className="text-xl sm:text-2xl font-bold text-amber-600 dark:text-amber-400">
            {myHighScore === null && isHighScoreLoading ? "…" : myHighScore}
          </div>
          <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">My Score</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-lg px-3 py-2 shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="text-xl sm:text-2xl font-bold text-blue-600 dark:text-blue-400">{moves}</div>
          <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Moves</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-lg px-3 py-2 shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="text-xl sm:text-2xl font-bold text-green-600 dark:text-green-400">{formatTime(gameTime)}</div>
          <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Time</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-lg px-3 py-2 shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="text-xl sm:text-2xl font-bold text-purple-600 dark:text-purple-400">{matchedPairs}/{CARD_SYMBOLS.length}</div>
          <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Pairs</div>
        </div>
      </div>
          ...
          <AnimatePresence>
            {cards.map((card, index) => (
              <motion.div
                ...
                className="relative"
                style={{ transformStyle: "preserve-3d", zIndex: isShuffling ? 10 : 1 }}
              >
                <MemoryCard
                  id={card.id}
                  value={card.value}
                  ...
                  disabled={flippedCards.length >= 2 || gameCompleted || isShuffling}
                />
              </motion.div>
            ))}
          </AnimatePresence>
      ...
      >
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-2">How to Play</h3>
        <p className="text-gray-600 dark:text-gray-300 text-sm">
          Click on cards to flip them over. Match all 10 pairs by remembering where each symbol is located. Try to complete the game in as few moves as possible!
        </p>
      </motion.div>
          ...
          >
            ...
            >
              ...
              <p className="text-gray-600 dark:text-gray-300 mb-4">
                You completed the memory game in {moves} moves and {formatTime(gameTime)}!
                <br />
