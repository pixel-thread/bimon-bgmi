...
export default function SnakeGame({
  ...
  const gameStateRef = useRef({ isPlaying: false, gameOver: false, showLifeLost: false });
  ...
  useEffect(() => {
    ...
    if (since < LIFE_REGENERATION_TIME) {
      ...
      const gain = Math.min(Math.floor(since / LIFE_REGENERATION_TIME), MAX_LIVES - lives);
      if (gain > 0) {
        setLives((p) => {
      ...
      if (lives + gain < MAX_LIVES) setLastLifeLoss(now - (since % LIFE_REGENERATION_TIME));
  ...
  const moveSnake = useCallback(() => {
    ...
    setSnake((curr) => {
      ...
      if (copy.slice(1).some((segment) => segment.x === head.x && segment.y === head.y)) {
        loseLife();
        return curr; // Don't move the snake
      }
      ...
      if (head.x === currentFood.x && head.y === currentFood.y) {
        ...
        setScore((p) => {
          ...
          if (ns === 50 || ns === 100 || ns === 150 || ns === 200) {
            const now = Date.now();
            if (!disableAds && (!lastAdTime || now - lastAdTime > 2 * 60 * 1000)) {
              setTimeout(() => {
  ...
  const queueDirectionChange = useCallback((newDir: { x: number; y: number }) => {
  ...
  const handleKey = useCallback(
    (e: KeyboardEvent) => {
      const arrows = ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', ' ', 'w', 'W', 'a', 'A', 's', 'S', 'd', 'D'];
  ...
  const handleTouchButton = useCallback((newDir: { x: number; y: number }) => {
    if (!gameStateRef.current.isPlaying) return;
    queueDirectionChange(newDir);
  }, [queueDirectionChange]);
  ...
  const startGame = () => {
    ...
    if (lives <= 0 && lastLifeLoss) {
      ...
      const regeneratedLives = Math.floor(timeSinceLastLoss / LIFE_REGENERATION_TIME);
      ...
      if (availableLives > 0) {
        ...
        const newLastLoss = lastLifeLoss + (regeneratedLives * LIFE_REGENERATION_TIME);
    ...
    <div className="snake-game-container flex flex-col items-center justify-center min-h-screen p-4">
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-4 sm:p-6 md:p-8 shadow-2xl border border-white/20 w-full max-w-2xl">
        ...
        <div className="text-center mb-4 sm:mb-6">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-2">Snake Game</h1>
          ...
          <div className="flex flex-wrap justify-center items-center gap-x-3 gap-y-2 text-white/80 text-sm sm:text-base">
            <div className="flex items-center space-x-2">
              ...
              <div className="flex space-x-1">
                {Array.from({ length: MAX_LIVES }).map((_, i) => (
                  <div
                    key={i}
                    className={`w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full ${i < lives ? 'bg-red-500' : 'bg-gray-700'}`}
                  />
                ))}
              </div>
            </div>
            ...
            <span>
              High: <span className="font-bold text-yellow-400">{highScore}</span>
            </span>
          </div>
          ...
          {lastLifeLoss && lives < MAX_LIVES && (
            <div className="text-xs text-white/60 mt-1">
              Next life in:{' '}
              {Math.max(0, Math.ceil((LIFE_REGENERATION_TIME - (Date.now() - lastLifeLoss)) / 60000))}{' '}
              min
            </div>
          )}
        ...
        <div className="relative mb-2 sm:mb-6 mx-auto w-full max-w-[min(95vw,500px)]">
          ...
          {showLifeLost && (
            <div className="absolute inset-0 bg-black/70 backdrop-blur-sm rounded-xl flex items-center justify-center">
              <div className="text-center text-white px-4">
                <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2">Life Lost!</h2>
                <p className="text-base sm:text-lg mb-4">Lives remaining: {lives}</p>
                <button
          ...
          {gameOver && (
            <div className="absolute inset-0 bg-black/70 backdrop-blur-sm rounded-xl flex items-center justify-center">
              <div className="text-center text-white px-4">
                <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2">Game Over!</h2>
                ...
                {(() => {
                  ...
                  if (availableLives > 0) {
                    ...
                    const minutesUntilNextLife = lastLifeLoss
                      ? Math.ceil((LIFE_REGENERATION_TIME - (currentTime - lastLifeLoss)) / 60000)
          ...
          {!isPlaying && !gameOver && !showLifeLost && (
            <div className="absolute inset-0 bg-black/70 backdrop-blur-sm rounded-xl flex items-center justify-center">
              <div className="text-center text-white px-4">
                <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-4">Ready to Play?</h2>
                <button
                  onClick={startGame}
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          <div className="text-center text-white/70 text-xs sm:text-sm">
            <div className="hidden md:block">Use arrow keys or WASD to move</div>
            <div className="md:hidden">Use buttons above to control the snake</div>
          </div>
        </div>
      </div>
