...
export default function LeaderboardModal({
  ...
  useEffect(() => {
    const loadScores = async () => {
      ...
      setIsLoading(true);
      try {
        const leaderboard = await gameScoreService.getLeaderboard('snake-game', 10);
          ...
          <ul className="space-y-2">
            {scores.map((entry, idx) => (
              <li
                ...
                className={`flex items-center justify-between px-3 py-2 rounded-lg ${idx === 0 ? 'bg-yellow-500/20 border border-yellow-400/50' : 'bg-white/5'
                  }`}
              >
                <div className="flex items-center gap-3">
                  <span className="text-white/70 font-semibold w-6">#{idx + 1}</span>
                  <span className="text-white font-medium">{entry.playerName || 'Anonymous'}</span>
                </div>
