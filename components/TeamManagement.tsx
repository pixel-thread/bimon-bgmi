...
export default function TeamManagement({
  ...
  useEffect(() => {
    if (tournaments.length > 0 && !selectedTournament) {
      ...
      getBestTournamentForAutoSelect(tournaments).then((bestTournamentId) => {
        if (bestTournamentId) {
        ...
        const sorted = [...tournaments].sort((a, b) => b.id.localeCompare(a.id));
                ...
                <div className="flex flex-col gap-2 w-full">
                  ...
                  <div className="flex flex-nowrap gap-1.5 items-center overflow-x-auto pb-1 -mx-1 px-1">
                    <imports.MatchDropdown
                      ...
                      options={[
                        "All",
                        ...Array.from({ length: matchCount }, (_, i) => (i + 1).toString()),
                      ]}
                      onSelect={setSelectedMatch}
                      onAddMatch={!readOnly && teams.length > 0 ? handleAddMatch : undefined}
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <imports.Button
                          ...
                          disabled={!selectedTournament || teams.length === 0 || selectedMatch === ""}
                          className="flex items-center gap-1.5 hover:bg-gray-100 transition-colors flex-shrink-0 px-2 py-1 text-xs sm:text-sm"
                        >
            ...
            {loading ? (
              <div className="flex justify-center items-center py-12">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">Hunting for teams...</p>
                </div>
              </div>
              ...
              <div className="text-center text-gray-500 py-4">
                ...
                <p className="text-lg font-medium mb-2">No teams match your filters</p>
                <p className="text-sm text-muted-foreground">Try adjusting your search or match selection</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                <AnimatePresence>
                  {sortedTeams.map(({ team, position }) => (
                    <motion.div key={team.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} transition={{ duration: 0.4 }}>
                      <imports.TeamCard
                        team={team}
                        searchTerm={searchTerm}
                        selectedMatch={selectedMatch}
                        onClick={!readOnly && selectedMatch !== "All" ? () => {
                          resetTempEdits();
                          setEditingTeam(team);
                        ...
                        tempEdits={tempEdits}
                        onTempEditChange={!readOnly ? handleTempEditChange : undefined}
                        position={position}
                        readOnly={readOnly}
                      />
                    </motion.div>
