...
import { getConfig, TournamentConfig } from "@/lib/config";
...
export default function TournamentEntryForm({ phoneNumber }: TournamentEntryFormProps) {
  ...
  useEffect(() => {
    ...
    const fetchTeamData = async () => {
      ...
      try {
        ...
        setError("Failed to load team data. Please refresh or try again later.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await setDoc(doc(db, "tournamentEntries", normalizedPhoneNumber), teamData);
      setIsExisting(true);
      setLastSubmitted(now);
      setSuccessMessage(isExisting ? "Team information updated successfully!" : "Team registered successfully!");
      setIsEditing(false);
    } catch (err) {
        ...
        <CardHeader className="pb-2">
          ...
          {isExisting && (
            <CardDescription className="text-center">
              {isEditing ? "Edit your team information" : "Your team is registered for the tournament"}
            </CardDescription>
          )}
        </CardHeader>
            ...
            <>
              ...
              {isExisting && !isEditing ? (
                <div className="space-y-6">
                  <div className="bg-blue-50 rounded-md p-4 border border-blue-100">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="font-semibold text-blue-800">Registration Details</h3>
                      <Button
                        variant="outline"
                        size="sm"
                  ...
                  <div className="bg-yellow-50 rounded-md p-4 border border-yellow-100">
                    <h3 className="font-semibold text-yellow-800 mb-2">Tournament Information</h3>
                    <p className="text-sm">
                      The tournament will start on {formatTournamentDate(tournamentConfig.startDate)}. You will receive
                      the room ID and password via SMS {tournamentConfig.notificationMinutesBefore} minutes before your
                      scheduled match.
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <h3 className="text-md font-semibold text-gray-700">Team Members</h3>
                    {playerIgns.map((ign, i) => (
                      <div key={i} className="space-y-2">
                        <Label htmlFor={`player-${i}`} className="text-sm font-medium">
                          {`Player ${i + 1} IGN`}
                  ...
                  <div className="flex gap-3">
                    ...
                    <Button type="submit" disabled={loading} className="flex-1 bg-blue-600 hover:bg-blue-700">
                      ...
                      {loading ? "Submitting..." : isExisting ? "Update Team" : "Register Team"}
                    </Button>
                  </div>
                </form>
              )}
