...
import { getBestTournamentForAutoSelect } from "@/lib/utils";
    ...
    const fetchTournamentsAndPlayers = async () => {
      ...
      try {
        ...
        if (tournamentsList.length > 0) {
          try {
            const bestTournamentId = await getBestTournamentForAutoSelect(tournamentsList);
                    ...
                    >
                      <td className="p-3">
                        <div className="flex items-center gap-3">
                          <Avatar
                            src={(player as any).avatarBase64 || (player as any).avatarUrl}
                            alt={player.name}
                            size="md"
                            className="w-9 h-9"
                          />
