...
interface SequentialEditModalProps {
  ...
  tempEdits: { [teamId: string]: { placement?: string; kills?: string; teamName?: string; playerKills?: string[]; playerParticipation?: boolean[] } };
...
export default function SequentialEditModal({
  ...
  useEffect(() => {
    ...
    const matches = sortedTeams
      .filter((team) =>
        ...
        team.players.some((player) => player.ign.toLowerCase().includes(lowerQuery))
      )
      .map((team) => team.id);
    ...
    if (matches.length > 0 && lastScrolledSearchRef.current !== playerSearch) {
      ...
      requestAnimationFrame(() => {
        ...
        if (element && scrollContainerRef.current) {
          ...
          element.classList.add("bg-yellow-100", "dark:bg-yellow-900", "transition-colors");
          setTimeout(() => {
            element.classList.remove("bg-yellow-100", "dark:bg-yellow-900", "transition-colors");
          }, 1500);
        }
      });
  ...
  const goToPrevMatch = () => {
    if (matchingTeamIds.length === 0) return;
    const prevIndex = (currentMatchIndex - 1 + matchingTeamIds.length) % matchingTeamIds.length;
  ...
  const getTeamHighlightState = (team: CombinedTeamData) => {
    ...
    const matchScore = team.matchScores?.[sequentialMatch] || { kills: 0, placementPoints: 0 };
    const savedPlacement =
      matchScore.placementPoints > 0 ? String(getPositionFromPoints(matchScore.placementPoints)).replace(/^0+/, "") : "0";
    const savedKills = matchScore.kills > 0 ? String(matchScore.kills).replace(/^0+/, "") : "0";
    const hasSavedData = matchScore.placementPoints > 0 || matchScore.kills > 0;

    const isEdited =
      teamEdits &&
      ((teamEdits.placement && teamEdits.placement.replace(/^0+/, "") !== savedPlacement) ||
  ...
  return (
    <Dialog open={showSequentialModal} onOpenChange={handleSequentialClose}>
      <DialogContent className="w-[95vw] max-w-[400px] sm:max-w-[500px] h-[85vh] sm:h-[90vh] p-3 sm:p-4 rounded-lg bg-white dark:bg-black overflow-hidden flex flex-col text-foreground">
        ...
        <div ref={scrollContainerRef} className="flex-1 mt-2 sm:mt-3 overflow-y-auto">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="relative w-20 sm:w-24">
                <input
                  ...
                  placeholder={window.innerWidth < 640 ? "Total" : "Total Players"}
                  className={`w-full h-8 sm:h-9 text-sm sm:text-base text-center rounded-md border ${totalKillsError ? "border-red-500" : "border-input dark:border-white/20"} focus:ring-2 focus:ring-blue-500 py-1 px-2 bg-background text-foreground touch-none`}
