  ...
  const filterTransactionsBySeason = useCallback(
    (
      ...
      return transactions.filter((transaction) => {
        ...
        const start = seasonData?.startDate ? toJSDate(seasonData.startDate) : null;
        if (start) {
          ...
          if (seasonData?.endDate) {
            ...
            end = seasonData?.isActive ? now : new Date(start.getTime() + 365 * 24 * 60 * 60 * 1000);
          }

          const isSameDayRange = !!seasonData?.endDate && !!start && !!end &&
