...
import { calculatePlayerKDs } from "@/lib/utils";
...
export default function TeamScoreInputs({
  ...
  useEffect(() => {
    if (showPlayerParticipation && players && players.length > 0 && onPlayerParticipationChange) {
      ...
      if (!playerParticipation || playerParticipation.length === 0) {
        ...
        if (JSON.stringify(smartDefaults) !== JSON.stringify(playerParticipation)) {
          onPlayerParticipationChange(smartDefaults);
        }
      }
    }
  }, [players, playerParticipation, onPlayerParticipationChange, showPlayerParticipation]);
  ...
  const handlePlayerKillsChange = (idx: number, value: string) => {
    ...
    newKills[idx] = value === "0" && (!playerKills[idx] || playerKills[idx] === "0") ? "" : value.slice(0, 2);
  ...
  const handlePlayerParticipationChange = (idx: number, participated: boolean) => {
      ...
      <div className="flex items-center gap-2 justify-end">
        <div className="relative w-14 sm:w-16 min-w-0">
          <Input
            ...
            className={`w-full h-9 sm:h-10 text-center rounded-[var(--radius-sm)] border ${error ? "border-destructive" : "border-input dark:border-white"} focus:ring-2 focus:ring-primary transition-all text-base sm:text-lg py-1 px-2 no-spinner touch-pinch-zoom-none`}
            inputMode="numeric"
            pattern="[0-9]*"
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        <Label htmlFor="placement" className="text-xs min-w-[80px]">Placement</Label>
        <Input
          id="placement"
          type="number"
          value={placement}
      ...
      {(showPlayerKills || showPlayerKD || showPlayerParticipation) && players.length > 0 && (
        <div>
          <Label className="block text-xs mb-1">Player Stats</Label>
          <div className="space-y-2">
            {players.map((player, idx) => {
              const participated = playerParticipation[idx] !== undefined ? playerParticipation[idx] : true;
              return (
                <div key={player.ign} className={`flex items-center gap-2 p-2 rounded-lg ${!participated ? 'opacity-60 bg-muted/30' : ''}`}>
                  <Label className="text-xs min-w-[80px] truncate">{player.ign}</Label>
                  
                  {showPlayerParticipation && (
                    <div className="flex flex-col items-center">
                      <Label className="text-xs text-muted-foreground mb-1">Played</Label>
                      <div className="flex items-center space-x-1">
                        <Checkbox
                          id={`participation-${idx}`}
                          checked={participated}
                          onCheckedChange={(checked) => handlePlayerParticipationChange(idx, checked as boolean)}
                        />
                        <Label htmlFor={`participation-${idx}`} className="cursor-pointer">
                          {participated ? (
                  ...
                  {showPlayerKills && (
                    <div className="flex flex-col items-center">
                      <Label className="text-xs text-muted-foreground mb-1">Kills</Label>
                      <Input
                        type="number"
                        value={playerKills && playerKills[idx] === "0" ? "" : playerKills?.[idx] || ""}
                        onChange={e => handlePlayerKillsChange(idx, e.target.value)}
                        min="0"
                        max="99"
                        className="w-12 text-sm text-center rounded-[var(--radius-sm)] border-input dark:border-white focus:ring-2 focus:ring-primary transition-all py-1 px-2 no-spinner touch-pinch-zoom-none"
                        inputMode="numeric"
      ...
      <div className="flex items-center gap-2 pt-2">
        <Label className="text-xs min-w-[80px] font-semibold">Total Kills</Label>
        <Input
          type="number"
          value={kills}
          onChange={e => onKillsChange(e.target.value)}
