...
export default function TeamCard({
  ...
  const score = matchScores[selectedMatch] || { kills: 0, placementPoints: 0, playerKills: [] };
  const totalScore = score.kills + score.placementPoints;
  const displayName = tempEdits[team.id]?.teamName || team.teamName;

  // Color-coded badges
  const placementColor = score.placementPoints >= 6
    ? "bg-green-100 text-green-800"
    : score.placementPoints >= 3
    ? "bg-yellow-100 text-yellow-800"
    : "bg-gray-100 text-gray-800";

  const killsColor = score.kills > 5
        ...
        {selectedMatch === "All" ? (
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-2">
            <div className="px-2 py-1 rounded-full text-xs font-medium bg-accent text-accent-foreground">
              Total Points: {Object.values(matchScores).reduce((acc, s) => acc + (s.kills || 0) + (s.placementPoints || 0), 0)}
            </div>
            <div
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-2">
            <div className={`px-2 py-1 rounded-full text-xs font-medium ${placementColor}`}>
              Placement: {score.placementPoints}
            </div>
            <div className={`px-2 py-1 rounded-full text-xs font-medium ${killsColor}`}>
              Kills: {score.kills}
            </div>
