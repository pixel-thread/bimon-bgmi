...
import { SUPER_ADMIN_EMAIL } from "@/config/adminAccess";
...
import { useLoadingState, LoadingOperation } from "@/lib/loadingStates";
import {
  useFormValidation,
  LoginValidationSchemas,
  validateEmail,
} from "@/lib/validation";
import { LoginRedirectService } from "@/lib/loginRedirectService";

function LoginContent() {
  const authState = useAuth();
  const { isAuthorized, loading, loginAsPlayer, loginAsPlayerWithSocial } = authState;
  ...
  const handlePlayerSocialAuth = async (providerId: string, useRedirect: boolean = false) => {
    ...
    try {
      ...
      loadingState.startLoading(LoadingOperation.GOOGLE_SIGNIN, undefined, operationId);
      ...
      if (result?.user) {
        ...
        try {
          ...
          const providerName = providerId.replace('.com', '').charAt(0).toUpperCase() + providerId.replace('.com', '').slice(1);
          ErrorHandler.showSuccess(`Successfully logged in with ${providerName}!`);
        } catch (error: any) {
          ...
          if (error.message?.includes("No player found with this social account")) {
  ...
  const handleSocialAuth = async (providerId: string, useRedirect: boolean = false) => {
    ...
    try {
      ...
      if (result?.user) {
        ...
        const emailDoc = await getDoc(doc(db, "authorized_emails", result.user.email!));
