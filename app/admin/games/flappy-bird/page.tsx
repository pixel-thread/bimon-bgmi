...
import { ArrowLeft, RotateCcw, Trophy, Users, GamepadIcon, AlertTriangle } from "lucide-react";
import Link from "next/link";
import { gameScoreService } from "@/lib/gameScoreService";
import { AuthGuard } from "@/components/AuthGuard";
import FlappyBird from "@/components/games/flappy-bird/FlappyBird";
...
const AdminFlappyBirdContent = () => {
  ...
  const loadData = async () => {
    ...
    try {
      // Load leaderboard
      const leaderboardData = await gameScoreService.getLeaderboard("flappy-bird", 50);
      setLeaderboard(leaderboardData);

      // Calculate stats and player statistics
      const scoresRef = collection(db, "gameScores");
      const q = query(scoresRef, where("gameId", "==", "flappy-bird"));
      const querySnapshot = await getDocs(q);
      
      const scores: number[] = [];
      const players = new Set<string>();
      const playerData = new Map<string, { scores: number[], name: string, lastPlayed?: Date }>();
      ...
      querySnapshot.docs.forEach(doc => {
        ...
        if (data.score > 0) {
          ...
          if (gameDate && (!player.lastPlayed || gameDate > player.lastPlayed)) {
      ...
      const playerStatsData: PlayerStats[] = Array.from(playerData.entries()).map(([playerId, data]) => {
        const playerScores = data.scores;
        return {
          playerName: data.name,
          gamesPlayed: playerScores.length,
          highestScore: Math.max(...playerScores),
          averageScore: Math.round(playerScores.reduce((a, b) => a + b, 0) / playerScores.length),
  ...
  const resetAllScores = async () => {
    if (!confirm("Are you sure you want to delete ALL Flappy Bird scores? This action cannot be undone!")) {
      return;
    }

    if (!confirm("This will permanently delete all player scores and leaderboard data. Type 'DELETE' if you're absolutely sure.")) {
      return;
    }

    setResetting(true);
    try {
      // Delete all game scores for flappy-bird from gameScores collection
      const scoresRef = collection(db, "gameScores");
      const q = query(scoresRef, where("gameId", "==", "flappy-bird"));
      const querySnapshot = await getDocs(q);
      ...
      const updatePromises = playersSnapshot.docs.map(async (playerDoc) => {
        ...
        if (playerData.gameScores?.["flappy-bird"]) {
          // Remove flappy-bird scores from player's gameScores
          const updatedGameScores = { ...playerData.gameScores };
          delete updatedGameScores["flappy-bird"];
      ...
      toast.success(`Successfully deleted ${deletePromises.length} score records and cleared player high scores`);
  ...
  // Reset lives for testing (dev-only feature)
  const resetLives = () => {
    localStorage.removeItem("flappy_bird_lives");
    localStorage.removeItem("flappy_bird_last_life_loss");
    toast.success("Lives reset to 5/5 for testing");
      ...
      <div className="container mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 w-full sm:w-auto">
            ...
            <div className="flex-1">
              <h1 className="text-2xl sm:text-3xl font-bold flex items-center gap-2 sm:gap-3">
                <GamepadIcon className="h-6 w-6 sm:h-8 sm:w-8 text-blue-500" />
                Flappy Bird Admin
              </h1>
        ...
        {stats && (
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
            <Card>
              ...
              <CardContent>
                <div className="text-lg sm:text-2xl font-bold">{stats.totalPlayers}</div>
              </CardContent>
            </Card>
            
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-xs sm:text-sm font-medium">Games Played</CardTitle>
                <GamepadIcon className="h-3 w-3 sm:h-4 sm:w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-lg sm:text-2xl font-bold">{stats.totalGamesPlayed}</div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-lg sm:text-2xl font-bold">{stats.highestScore}</div>
              </CardContent>
            </Card>
        ...
        <Tabs defaultValue="management" className="space-y-4 sm:space-y-6">
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="statistics" className="text-xs sm:text-sm">Player Stats</TabsTrigger>
            <TabsTrigger value="game" className="text-xs sm:text-sm">Test Game</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="management" className="space-y-4 sm:space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                ...
                <CardContent className="space-y-4">
                  <div className="p-3 sm:p-4 border border-red-200 bg-red-50 dark:bg-red-950 dark:border-red-800 rounded-lg">
                    ...
                    <p className="text-xs sm:text-sm text-red-600 dark:text-red-300 mb-3">
                      This will permanently delete all player scores and reset the leaderboard. 
                      Current data: {stats?.totalGamesPlayed || 0} games from {stats?.totalPlayers || 0} players.
                    </p>
                    <Button 
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base sm:text-lg">Development Tools</CardTitle>
                  <CardDescription className="text-xs sm:text-sm">
                    Testing utilities for development
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="p-3 sm:p-4 border border-blue-200 bg-blue-50 dark:bg-blue-950 dark:border-blue-800 rounded-lg">
                    ...
                    <p className="text-xs sm:text-sm text-blue-600 dark:text-blue-300 mb-3">
                      Reset the lives system to 5/5 for testing game mechanics. This only affects your current browser session.
                    </p>
                    <Button 
                      onClick={resetLives}
                      variant="outline"
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                  <span className="text-base sm:text-lg">Current Leaderboard</span>
              ...
              <CardContent>
                {loading ? (
                  <div className="text-center py-8">
                    ...
                    <p className="mt-2 text-muted-foreground text-sm">Loading leaderboard...</p>
                  </div>
                    ...
                    {leaderboard.map((entry, index) => (
                      ...
                      >
                        <div className="flex items-center gap-2 sm:gap-3">
                          ...
                          <span className="font-medium text-sm sm:text-base truncate">{entry.playerName}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-base sm:text-lg">{entry.score}</span>
                          {entry.timestamp && (
          ...
          <TabsContent value="statistics">
            <Card>
              <CardHeader>
                <CardTitle className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                  <span className="text-base sm:text-lg">Player Statistics</span>
                  <Badge variant="secondary" className="text-xs">
              ...
              <CardContent>
                {loading ? (
                  <div className="text-center py-8">
                    <div className="animate-spin h-6 w-6 sm:h-8 sm:w-8 border-b-2 border-primary rounded-full mx-auto"></div>
                    <p className="mt-2 text-muted-foreground text-sm">Loading player statistics...</p>
                  </div>
                    ...
                    {playerStats.map((player, index) => (
                      ...
                      >
                        <div className="flex items-center gap-2 sm:gap-3">
                          ...
                          <div>
                            <span className="font-medium text-sm sm:text-base block">{player.playerName}</span>
                            {player.lastPlayed && (
                              <span className="text-xs text-muted-foreground">
                                Last played: {player.lastPlayed.toLocaleDateString()}
                              </span>
                            )}
                          </div>
                        </div>
          ...
          <TabsContent value="game" className="p-0">
            <div className="w-full">
              <div className="mb-4 p-4 bg-muted/50 rounded-lg">
                <h3 className="font-semibold text-sm sm:text-base mb-2">Full Game Experience</h3>
                <p className="text-xs sm:text-sm text-muted-foreground">
                  This shows exactly how players see the game - no borders or modifications
                </p>
              </div>
              <FlappyBird />
            </div>
...
export default function AdminFlappyBirdPage() {
  return (
    <AuthGuard requireAuth={true}>
      <AdminFlappyBirdContent />
    </AuthGuard>
  );
}
