...
const AdminSnakeContent = () => {
  ...
  const loadData = async () => {
    ...
    try {
      // Load leaderboard
      const leaderboardData = await gameScoreService.getLeaderboard("snake-game", 50);
      setLeaderboard(leaderboardData);

      // Calculate stats and player statistics
      const scoresRef = collection(db, "gameScores");
      const q = query(scoresRef, where("gameId", "==", "snake-game"));
  ...
  const resetAllScores = async () => {
    if (!confirm("Are you sure you want to delete ALL Snake Game scores? This action cannot be undone!")) {
      return;
    }
    ...
    setResetting(true);
    try {
      // Delete all game scores for snake-game from gameScores collection
      const scoresRef = collection(db, "gameScores");
      const q = query(scoresRef, where("gameId", "==", "snake-game"));
      const querySnapshot = await getDocs(q);
      ...
      const updatePromises = playersSnapshot.docs.map(async (playerDoc) => {
        ...
        if (playerData.gameScores?.["snake-game"]) {
          // Remove snake-game scores from player's gameScores
          const updatedGameScores = { ...playerData.gameScores };
          delete updatedGameScores["snake-game"];
  ...
  const resetLives = () => {
    localStorage.removeItem("snake_game_lives");
    localStorage.removeItem("snake_game_last_life_loss");
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 w-full sm:w-auto">
            ...
            <div className="flex-1">
              <h1 className="text-2xl sm:text-3xl font-bold flex items-center gap-2 sm:gap-3">
                <GamepadIcon className="h-6 w-6 sm:h-8 sm:w-8 text-green-500" />
                Snake Game Admin
              </h1>
