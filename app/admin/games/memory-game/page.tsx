...
import { ArrowLeft, RotateCcw, Trophy, Users, GamepadIcon, AlertTriangle, Clock, Target } from "lucide-react";
...
const AdminMemoryGameContent = () => {
  ...
  const loadData = async () => {
    ...
    try {
      // Load leaderboard
      const leaderboardData = await gameScoreService.getLeaderboard("memory-game", 50);
      ...
      const playerStatsData: PlayerStats[] = Array.from(playerData.entries()).map(([playerId, data]) => {
        ...
        return {
          ...
          averageMoves: playerMoves.length > 0 ? Math.round(playerMoves.reduce((a, b) => a + b, 0) / playerMoves.length) : 0,
          averageTime: playerTimes.length > 0 ? Math.round(playerTimes.reduce((a, b) => a + b, 0) / playerTimes.length) : 0,
  ...
  const resetAllScores = async () => {
    if (!confirm("Are you sure you want to delete ALL Memory Game scores? This action cannot be undone!")) {
      return;
    }

    if (!confirm("This will permanently delete all player scores and reset the leaderboard. Type 'DELETE' if you're absolutely sure.")) {
      return;
    }
