...
export async function POST(req: NextRequest) {
  ...
  console.log(`Login attempt for ${normalizedPhoneNumber} with OTP: ${password}`);
  ...
  if (userData.password !== password) {
    console.log(`Invalid OTP for ${normalizedPhoneNumber}. Expected: ${userData.password}, Got: ${password}`);
  ...
  if (now > expiresAt) {
    console.log(`OTP expired for ${normalizedPhoneNumber}. Expires at: ${userData.expiresAt}`);
