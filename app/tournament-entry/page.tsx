...
export default async function TournamentEntryPage({ searchParams }: PageProps) {
  ...
  if (!phoneNumber || (Array.isArray(phoneNumber) ? !/^\d{10}$/.test(phoneNumber[0]) : !/^\d{10}$/.test(phoneNumber))) {
    redirect('/login?error=invalid_phone');
  }

  const validatedPhoneNumber = Array.isArray(phoneNumber) ? phoneNumber[0] : phoneNumber;

  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-gray-600">Loading tournament entry form...</div>}>
